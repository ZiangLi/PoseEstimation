...
public:
	...
	DLLEXPORT CxlibFaceDetector(EnumViewAngle viewAngle /*= VIEW_ANGLE_FRONTAL*/, EnumFeaType feaType = FEA_SURF, const char* xmlfile = NULL); 
	DLLEXPORT ~CxlibFaceDetector(void);
	
	// init
	DLLEXPORT void init( EnumViewAngle viewAngle = VIEW_ANGLE_FRONTAL, EnumFeaType feaType = FEA_SURF, const char* xmlfile = NULL);
...
//	DLLEXPORT void config( tagDetectConfig configParam = tagDetectConfig());
	...
	DLLEXPORT  void SetFaceDetectionSizeRange(IplImage* lpimage);
...
public:
	...
	DLLEXPORT CxlibFaceAnalyzer(EnumViewAngle viewAngle, EnumTrackerType traType = TRA_SURF, 
		bool bBlink=true, bool bSmile=true, bool bGender=true, bool bAge=true, bool bFaceRecog =true, 
		int sampleRate = 4, const char *str_facesetxml =NULL, int recognierType = RECOGNIZER_CAS_GLOH, bool bEnableAutoCluster = false,
		bool bLandmarkRegressor = false);
	DLLEXPORT ~CxlibFaceAnalyzer(void);
	
	// init
	DLLEXPORT void init(EnumViewAngle viewAngle = VIEW_ANGLE_FRONTAL, EnumTrackerType traType = TRA_SURF, 
		bool bBlink=true, bool bSmile=true, bool bGender=true, bool bAge=true, bool bFaceRecog =true, 
		int sampleRate = 4, const char *str_facesetxml =NULL,int recognierType = RECOGNIZER_CAS_GLOH, bool bEnableAutoCluster = false, bool bLandmarkRegressor = false);

	DLLEXPORT void setMouseParam(CxMouseParam *param);
	...
	DLLEXPORT int       getFaceBlink(int idx);     
	DLLEXPORT int       getFaceSmile(int idx);     
	DLLEXPORT float     getFaceSmileProb(int idx); 
	DLLEXPORT int       getFaceGender(int idx);    
	DLLEXPORT int       getFaceAge(int idx);       
	DLLEXPORT int       getFaceID(int idx);      
	DLLEXPORT float     getFaceProb(int idx);      
	DLLEXPORT char     *getFaceName(int idx);

	DLLEXPORT int       getFaceFemaleNum();        
	DLLEXPORT int       getFaceMaleNum();          

	DLLEXPORT int       getFaceBabyNum();          
	DLLEXPORT int       getFaceKidNum();           
	DLLEXPORT int       getFaceAdultNum();         
	DLLEXPORT int       getFaceSeniorNum();       
	DLLEXPORT int       getFaceSmileNum();         
